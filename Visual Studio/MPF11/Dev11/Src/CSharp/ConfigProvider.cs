...
namespace Microsoft.VisualStudio.Project
{
    ...
    [ComVisible(true)]
    public abstract class ConfigProvider : IVsCfgProvider2, IEnumerable<Config>
    {
        ...
        private Dictionary<string, Config> configurationsList = new Dictionary<string, Config>();
        ...
        /// <param name="platformName">The platform name of the configuraiton</param>
        /// <returns>An instance of a ProjectConfig object.</returns>
        protected virtual Config GetProjectConfiguration(string configName, string platformName)
        {
            ...
            Config requestedConfiguration = CreateProjectConfiguration(configName, platformName);
            configurationsList.Add(configName, requestedConfiguration);

            return requestedConfiguration;
        }

        protected abstract Config CreateProjectConfiguration(string configName, string platformName);
        ...
        public virtual int GetCfgs(uint celt, IVsCfg[] a, uint[] actual, uint[] flags)
        {
            ...
            int i = 0;
            string[] configList = GetPropertiesConditionedOn(ProjectFileConstants.Configuration);
            string[] platformList = GetPropertiesConditionedOn(ProjectFileConstants.Platform);
            ...
            if (a != null)
            {
                foreach (string platformName in platformList)
                {
                    ...
                    {
                        var config = this.GetProjectConfiguration(configName, platformName);
                        if (config == null)
                            continue;
        ...
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
        public abstract int GetPlatformNames(uint celt, string[] names, uint[] actual);
        ...
        /// <param name="actual">The actual number of platform names returned.</param>
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
        public abstract int GetSupportedPlatformNames(uint celt, string[] names, uint[] actual);
        ...
        protected virtual string[] GetPropertiesConditionedOn(string constant)
        {
            ...
            Debug.Assert(configurations != null, "If there are no configurations existant, we can't show the property pages of the project.");
