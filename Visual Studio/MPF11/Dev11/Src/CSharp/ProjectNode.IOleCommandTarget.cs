...
namespace Microsoft.VisualStudio.Project
{
    ...
    public abstract partial class ProjectNode {
        ...
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
        [SuppressMessage("Microsoft.Naming", "CA1702:CompoundWordsShouldBeCasedCorrectly", MessageId = "vaIn")]
        protected virtual int publicExecCommand(Guid cmdGroup, uint cmdId, uint cmdExecOpt, IntPtr vaIn, IntPtr vaOut, CommandOrigin commandOrigin)
        {
            if (IsClosed)
            {
                return (int)OleConstants.OLECMDERR_E_NOTSUPPORTED;
        ...
        protected virtual int QueryStatusSelection(Guid cmdGroup, uint cCmds, OLECMD[] prgCmds, IntPtr pCmdText, CommandOrigin commandOrigin)
        {
            ...
            VsUtilities.ArgumentNotNull("prgCmds", prgCmds);
        ...
        {
            return this.publicExecCommand(guidCmdGroup, nCmdId, nCmdExecOpt, pvaIn, pvaOut, CommandOrigin.OleCommandTarget);
        }
